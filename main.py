...
def RequestsList():
    ...
    trv.heading("2", text="Asset Name")
    trv.heading("3", text="Request Status")
    trv.heading("4", text="Asset Type")


    id = employee.id
    data = employee.get_all_Requests(id)
...
def personelInventoryList():
    ...
    # Main Heading
    main_heading = Label(personelInventoryFrame, text="My Current Assets", font=('Calibri', 25, "bold"))
    main_heading.grid(row=0, column=0, columnspan=5, padx=(20, 20), sticky="n")

    # Second Label
    second_label = Label(personelInventoryFrame, text="Please have a look at your current assets list", font=('Calibri', 15))
    second_label.grid(row=1, column=0, columnspan=5, padx=(20, 20), pady=10, sticky="n")
    ...
    trv.heading("1", text="ID")
    trv.heading("2", text="Asset Name")
    trv.heading("3", text="Asset Type")
    trv.heading("4", text="Status")
    trv.heading("5", text="Employee Assigned")

    id = employee.id
    data = employee.get_all_personel_assets(id)
    ...
    def returned():
        check = employee.update_asset_status(selected, 'Available', 0)
        if check:
...
def userInventoryList():
    ...
    # Main Heading
    main_heading = Label(userInventoryFrame, text="Inventory List", font=('Calibri', 25, "bold"))
    main_heading.grid(row=0, column=0, columnspan=5, padx=(20, 20), sticky="n")

    # Second Label
    second_label = Label(userInventoryFrame, text="Please have a look at your inventory list", font=('Calibri', 15))
    second_label.grid(row=1, column=0, columnspan=5, padx=(20, 20), pady=10, sticky="n")
    ...
    trv.heading("3", text="Asset type")
    trv.heading("4", text="Status")
    trv.heading("5", text="Timming")

    data = employee.get_all_assets()
    ...
    def assigned():
        duplicate=employee.check_dublicate_request(selected, employee.id)
        if duplicate:
            messagebox.showerror('Error', 'Your request is already under process!')
        else:
            if status =='New' or status=='Available':

                check = employee.request(selected, employee.id)
                if check:
...
def userMenu():
    ...
    lbl_username = Label(userMenuFrame, text="Welcome User ", fg='black', font=('Calibri', 25, "bold"), bd=18)
    lbl_username.grid(row=1, pady=30)

    b1 = ttk.Button(userMenuFrame, style="Rounded.TButton", text="Inventory List", width=20, command=toggletoUInventory, cursor="hand2")
    b1.grid(row=3, pady=10)
    b2 = ttk.Button(userMenuFrame, style="Rounded.TButton", text="My Assets List", width=20, command=toggleToPersonelInventory, cursor="hand2")
...
def userRegisteration():
    ...
    lbl = Label(userRegFrame, text="User Registeration", fg='black', font=('Calibri', 25,"bold"))
    ...
    def registeration_function():
        ...
        if name == '' or gender == '' or email == '' or password == '':
            ...
            check = employee.register(name, password, gender, email)
            if check:
                toggleToUserLogin()
            else:
...
def userLogin():
    ...
    lbl = Label(userFrame, text="Dear User", fg='black', font=('Calibri', 25, "bold"))
    lbl.grid(row=0, pady=2, columnspan=2, padx=60)
    ...
    def login_function():
        email = email_entry.get()
        password = password_entry.get()
        check = employee.login(email, password)
        if check:
            toggleToUserMenu()
            if employee.get_new_messages(employee.id):
                notification.notify('Message!', 'Request Response by Admin!')
                employee.update_message_status(employee.id)
        else:
...
def inventoryList():
    ...
    # Main Heading
    main_heading = Label(inventoryFrame, text="Inventory List", font=('Calibri', 25, "bold"))
    main_heading.grid(row=0, column=0, columnspan=5, padx=(20, 20), sticky="n")

    # Second Label
    second_label = Label(inventoryFrame, text="Please have a look at your inventory list", font=('Calibri', 15))
    second_label.grid(row=1, column=0, columnspan=5, padx=(20, 20), pady=10, sticky="n")
...
def newItem():
    ...
    typechoosen['values'] = ("Computers", "Laptops", "Mobile Devices", "Other")
    ...
    statuschoosen['values'] = ("Out of order", "New")
...
def adminMenu():
    ...
    adminMenuFrame.pack(side=TOP, pady=10)

    lbl_username = Label(adminMenuFrame, text="Welcome Admin",fg='black', font=('Calibri', 25,"bold"), bd=18)
    ...
    b1 = ttk.Button(adminMenuFrame, style="Rounded.TButton", text="View Inventory List", width=30, command=toggleToInventoryList, cursor="hand2")
    b1.grid(row=3, pady=10)
    b2 = ttk.Button(adminMenuFrame, style="Rounded.TButton", text="View Requests List", width=30, command=toggleToRequests, cursor="hand2")
    b2.grid(row=4, pady=10)
    b3 = ttk.Button(adminMenuFrame, style="Rounded.TButton", text="Generate Assets CSV file", width=30, command=assetsFile, cursor="hand2")
    b3.grid(row=5, pady=10)
    b4 = ttk.Button(adminMenuFrame, style="Rounded.TButton", text="Generate Users CSV file", width=30, command=usersFile, cursor="hand2")
    b4.grid(row=7, pady=10)
    b5 = ttk.Button(adminMenuFrame, style="Rounded.TButton", text="Logout", width=30, command=toggleBackToMain, cursor="hand2")
...
def adminLogin():
    ...
    lbl = Label(adminFrame, text="Dear Admin", fg='black', font=('Calibri', 25, "bold"))
...
def mainForm():
    ...
    lbl_username = Label(mainFrame, text="Asset Inventory Management System",fg='black', font=('Calibri', 25, "bold"))
    ...
    b1 = ttk.Button(mainFrame, style="Rounded.TButton", text="Admin", width=20, command=toggleToAdmin, cursor="hand2")
    b1.grid(row=9, pady=10)

    b2 = ttk.Button(mainFrame, style="Rounded.TButton", text="User", width=20, command=toggleToUser, cursor="hand2")
